...
В основе всех принципов разработки bem-components лежит опыт работы нашей команды над другими библиотеками. Поэтому при разарботке билиотеки мы старались взять все лучшее из прошлого, на его базе строить новое, и при этом не повторять прежних ошибок.
...
Запускаем точечную сборку на основе путей в файловой системе:

```bash
$ magic make desktop.examples/button desktop.tests/button
```

Запускаем точечную сборку по названию задачи:

```bash
$ magic run specs tmpl-specs
```
